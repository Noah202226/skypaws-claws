...
export default function PetDetailModal({ pet, isOpen, onClose }: any) {
  ...
  const {
    ...
    fetchInstallments,
    addInstallment,
    deleteInstallment, // Added this from store
  ...
  const handlePaymentSubmit = async (txId: string) => {
    ...
    try {
      ...
      // Refresh transactions to update the 'Remaining Balance' in the UI
      fetchTransactions(pet.$id);
    } catch (error) {
  ...
  const handleDeletePayment = async (instId: string, txId: string) => {
    if (confirm("Delete this payment? The balance will be restored.")) {
      try {
        ...
        fetchTransactions(pet.$id); // Refresh main list balance
      } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-110 flex items-center justify-center p-0 md:p-6">
      ...
      <div className="relative w-full max-w-7xl h-full md:h-[90vh] bg-slate-900 border-x md:border border-slate-800 md:rounded-[3rem] shadow-2xl overflow-hidden flex flex-col">
        {/* HEADER */}
        <div className="relative p-6 md:p-10 pb-6 flex justify-between items-center border-b border-slate-800/50 bg-linear-to-b from-slate-800/20 to-transparent">
          <div className="flex items-center gap-5">
            <div className="h-16 w-16 md:h-20 md:w-20 rounded-3xl bg-indigo-600 flex items-center justify-center shadow-[0_0_30px_rgba(79,70,229,0.3)]">
          ...
          <button
            onClick={onClose}
            className="p-3 bg-slate-800/50 hover:bg-rose-500/20 hover:text-rose-500 rounded-2xl text-slate-400 transition-all"
          >
            <X size={24} />
          </button>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
              ...
              <div className="lg:col-span-8 space-y-6">
                <h4 className="text-xs font-black text-white uppercase tracking-[0.2em] flex items-center gap-3">
                  <History size={16} className="text-indigo-500" /> Medical &
                  Service Logs
                </h4>
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-4">
                    {transactions.map((tx: any) => (
                      <div
                        key={tx.$id}
                        className="group relative bg-slate-950/60 border border-slate-800/50 rounded-3xl p-6 hover:border-indigo-500/40 transition-all"
                      >
                        {tx.balanceRemaining <= 0 && (
                          <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.03] rotate-[-15deg]">
                            <span className="text-4xl font-black uppercase border-2 border-white px-10 py-2 rounded-3xl">
                              Settled
                            </span>
                          </div>
                        )}
                        ...
                        <div className="absolute top-6 right-6 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            className="p-2 bg-slate-900 border border-slate-800 hover:border-indigo-500/50 hover:text-indigo-400 rounded-xl text-slate-500 transition-all"
                          >
                            <Edit3 size={14} />
                          </button>
                          <button
                            onClick={() => handleDelete(tx.$id)}
                            className="p-2 bg-slate-900 border border-slate-800 hover:border-rose-500/50 hover:text-rose-500 rounded-xl text-slate-500 transition-all"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
                        ...
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex items-center gap-4">
                            <div className="h-12 w-12 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col items-center justify-center text-blue-400">
                              <Scale size={12} />
                            ...
                            <div>
                              <h5 className="font-black text-white text-lg uppercase italic">
                        ...
                        {tx.notes && (
                          <div className="mb-6 p-4 bg-slate-900/30 rounded-2xl border border-slate-800/30">
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-800/50 flex flex-wrap items-center justify-between gap-4">
                          ...
                          <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-indigo-600/5 hover:bg-indigo-600/10 border border-indigo-500/20 rounded-xl text-[10px] font-black text-indigo-400 uppercase tracking-widest transition-all"
                          >
                            <Receipt size={14} />{" "}
                            {expandedTx === tx.$id
                              ? "Close History"
                              ...
                              {loadingInstallments ? (
                                ...
                                <div className="space-y-2">
                                  {installments[tx.$id]?.map((inst: any) => (
                                    <div
                                      key={inst.$id}
                                      className="group/inst flex justify-between items-center p-3 bg-slate-900/50 rounded-2xl hover:bg-slate-900 transition-colors"
                                    >
                                      <div className="flex items-center gap-3">
                                        <Banknote
                                          size={14}
                                      ...
                                      <div className="flex items-center gap-4">
                                        ...
                                        <button
                                          ...
                                          className="opacity-0 group-hover/inst:opacity-100 p-1.5 hover:bg-rose-500/10 hover:text-rose-500 text-slate-600 rounded-lg transition-all"
                                        >
                                          <Trash2 size={12} />
                                        </button>
                                      </div>
                                  ...
                                  {tx.balanceRemaining > 0 && (
                                    <div className="space-y-2 pt-4 border-t border-slate-800/50 mt-2">
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                        <input
                                          type="date"
                                          className="bg-slate-950 border border-slate-800 rounded-xl py-2 px-3 text-[10px] text-white"
                                          value={payDate}
                                          onChange={(e) =>
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
                                        <div className="md:col-span-5 relative">
                                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-[10px]">
                                            ₱
                                          </span>
                                          <input
                                            type="number"
                                            placeholder="Amount"
                                            className="w-full bg-slate-950 border border-slate-800 rounded-xl py-2 pl-7 pr-3 text-[10px] text-white"
                                            value={payAmount}
                                            onChange={(e) =>
                                        ...
                                        <select
                                          className="md:col-span-4 bg-slate-950 border border-slate-800 rounded-xl py-2 px-3 text-[10px] text-white"
                                          value={payMethod}
                                          onChange={(e) =>
                                        ...
                                        <button
                                          ...
                                          className="md:col-span-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-[9px] font-black uppercase"
                                        >
                                          Add Payment
                                        </button>
                                      </div>
              ...
              <div className="lg:col-span-4 space-y-6">
                <div className="bg-indigo-600 rounded-[2.5rem] p-8 shadow-xl text-white relative overflow-hidden group">
                  ...
                  <Button
                    ...
                    className="w-full bg-white text-indigo-600 hover:bg-slate-100 font-black uppercase text-[10px] tracking-widest h-14 rounded-2xl"
                  >
...
function VitalCard({ icon, label, value }: any) {
  return (
    <div className="bg-slate-950/50 border border-slate-800/50 rounded-3xl p-6">
      ...
      <p className="text-xl font-black text-white mt-1 uppercase">{value}</p>
    </div>
  );
}
    ...
    <div className="flex flex-col">
      <span
        className={`text-[9px] font-bold uppercase ${
          color === "text-white" ? "text-slate-500" : color
        }`}
      >
        {label}
      </span>
...
function SummaryRow({ label, value, color = "text-white" }: any) {
  return (
